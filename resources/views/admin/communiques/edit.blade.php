...
@section('title', 'Modifier un Communiqué')
...
<form action="{{ route('admin.communiques.update', $communique) }}" method="POST" class="interactive-dashboard-form">
    @csrf
    @method('PUT')
                        ...
                        <div class="form-card-body">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <textarea class="form-control" id="content" name="content" rows="4" required>{{ old('content', $communique->content) }}</textarea>
                                </div>
                    ...
                    <div class="form-card h-100">
                        ...
                        <div class="form-card-body">
                            <div class="communique-preview-box mb-3">
                                ...
                                <p id="preview-text" class="mb-0 opacity-90">{{ $communique->content }}</p>
                            </div>
            ...
            <div class="form-card">
                ...
                <div class="form-card-body row">
                    <div class="col-md-4 form-group">
                        ...
                        <div class="input-group">
                            ...
                            <select class="form-select" id="target_audience" name="target_audience" required>
                                @foreach(\App\Models\Communique::TARGETS as $key => $label)
                                    <option value="{{ $key }}" {{ $communique->target_audience === $key ? 'selected' : '' }}>
                                        {{ $label }}
                                        @if($key === 'all')
                        ...
                        <small class="form-text mt-2" style="color: #6c757d !important; display: block;">La cible déterminée lors de la création (pour info).</small>
                    </div>
                    ...
                    <div class="col-md-4 form-group">
                        ...
                        <input type="datetime-local" class="form-control" id="end_at" name="end_at" value="{{ $communique->end_at ? $communique->end_at->format('Y-m-d\TH:i') : '' }}">
                    </div>
                </div>
                <div class="form-card-body row border-top pt-3 mt-0">
                    <div class="col-md-6 form-group">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" class="form-control" id="order" name="order" value="{{ old('order', $communique->order) }}" min="0">
                        </div>
                    ...
                    <div class="col-md-6 form-group d-flex align-items-center pt-4">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" name="is_active" id="isActiveEdit" {{ old('is_active', $communique->is_active) ? 'checked' : '' }} style="width: 3em; height: 1.5em;">
                            <label class="form-check-label ms-3 fw-bold" for="isActiveEdit">Publié</label>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="form-footer mt-4 d-flex justify-content-between align-items-center">
        ...
        <button type="submit" class="btn btn-success px-5" id="submitBtn">
            <span class="normal-state"><i class="fas fa-save me-2"></i>Mettre à jour</span>
            <span class="loading-state d-none"><i class="fas fa-spinner fa-spin me-2"></i>Traitement...</span>
        </button>
    </div>
</form>
...
<script>
    ...
    document.getElementById('content').addEventListener('input', function(e) {
        ...
        previewText.textContent = e.target.value || "{{ $communique->content }}";
    });
