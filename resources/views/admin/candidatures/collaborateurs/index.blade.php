...
@section('title', 'Candidatures - Collaborateurs')
...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-white">Candidatures Collaborateurs</h1>
    ...
    <div class="card" style="background-color: #1e293b; border: 1px solid #334155;">
        <div class="card-header" style="background-color: #0f172a; border-bottom: 1px solid #334155;">
            <h5 class="mb-0 text-white"><i class="fas fa-list me-2"></i>Liste des candidatures</h5>
        </div>
        <div class="card-body">
                    ...
                    <tbody>
                        @forelse($candidatures as $c)
                            <tr>
                                ...
                                <td>{{ $c->telephone }}</td>
                                <td>{{ $c->poste }}</td>
                                <td>{{ $c->statut ?? 'nouveau' }}</td>
                                <td>{{ optional($c->created_at)->format('d/m/Y H:i') }}</td>
                                <td class="text-end">
                                    <a class="btn btn-sm btn-info" href="{{ route('admin.candidatures.collaborateurs.show', $c->id) }}">Voir</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="8" class="text-center text-muted py-4">Aucune candidature</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
