...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div>
            <a href="{{ route('admin.articles.actualites.edit', $actualite->id) }}" class="btn btn-warning">
    ...
    <div class="row">
        ...
        <div class="col-lg-8">
            ...
            <div class="card modern-card mb-4">
                ...
                <div class="card-body">
                    <div class="event-excerpt mb-4">
                        <p class="lead text-white">{{ $actualite->excerpt }}</p>
                    </div>
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-12 mb-3">
                            <label class="text-muted small">Catégorie de l'actualité</label>
                            <p class="text-white mb-0">
                                ...
                                @endphp
                                <span class="badge bg-{{ $category['color'] }}" style="font-size: 1rem; padding: 0.5rem 1rem;">
            ...
            <!-- SEO -->
            @if($actualite->meta_title || $actualite->meta_description || $actualite->meta_keywords)
            <div class="card modern-card mb-4">
                <div class="card-header">
                    <h5 class="mb-0"><i class="fas fa-search me-2"></i>Informations SEO</h5>
                </div>
                <div class="card-body">
                    @if($actualite->meta_title)
                    <div class="mb-3">
                        <label class="text-muted small">Meta Title</label>
                        <p class="text-white mb-0">{{ $actualite->meta_title }}</p>
                    </div>
                    @endif
                    @if($actualite->meta_description)
                    <div class="mb-3">
                        <label class="text-muted small">Meta Description</label>
                        <p class="text-white mb-0">{{ $actualite->meta_description }}</p>
                    </div>
                    @endif
                    @if($actualite->meta_keywords)
                    <div class="mb-3">
                        <label class="text-muted small">Mots-clés</label>
                        <p class="text-white mb-0">{{ $actualite->meta_keywords }}</p>
                    </div>
                    @endif
                </div>
            </div>
        ...
        <div class="col-lg-4">
            ...
            <div class="card modern-card mb-4">
                ...
                <div class="card-body">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <p class="text-white mb-0">{{ $actualite->published_at->format('d/m/Y à H:i') }}</p>
                    </div>
                    @endif
                    ...
                    <div class="mb-3">
                        ...
                        <p class="text-white mb-0">
                            <i class="fas fa-eye me-2"></i>{{ number_format($actualite->views_count) }}
                        </p>
                    </div>
                    ...
                    <div class="d-grid gap-2">
                        <form action="{{ route('admin.articles.actualites.toggle-status', $actualite->id) }}" method="POST">
                            @csrf
                            @method('PATCH')
                            <button type="submit" class="btn btn-sm btn-outline-{{ $actualite->status === 'published' ? 'warning' : 'success' }} w-100">
                                @if($actualite->status === 'published')
                        ...
                        <form action="{{ route('admin.articles.actualites.toggle-featured', $actualite->id) }}" method="POST">
                            @csrf
                            @method('PATCH')
                            <button type="submit" class="btn btn-sm btn-outline-{{ $actualite->is_featured ? 'secondary' : 'success' }} w-100">
                                @if($actualite->is_featured)
            ...
            <div class="card modern-card mb-4">
                ...
                <div class="card-body">
                    ...
                    @if($actualite->visibility === 'specific' && $actualite->formations)
                    <div class="mb-3">
                        <label class="text-muted small">Formations concernées</label>
                        <ul class="list-unstyled mb-0">
                            @foreach($actualite->formations as $formationId)
                                @php
            ...
            <div class="card modern-card mb-4">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <p class="text-white mb-0"><code>{{ $actualite->slug }}</code></p>
                    </div>
                    <div class="mb-3">
                        <label class="text-muted small">ID</label>
                        <p class="text-white mb-0">{{ $actualite->id }}</p>
                    </div>
                    <div class="mb-3">
                        <label class="text-muted small">Créé le</label>
                        <p class="text-white mb-0">{{ $actualite->created_at->format('d/m/Y à H:i') }}</p>
                    </div>
                    <div class="mb-3">
                        <label class="text-muted small">Dernière modification</label>
                        <p class="text-white mb-0">{{ $actualite->updated_at->format('d/m/Y à H:i') }}</p>
                    </div>
                </div>
            </div>
        </div>
