...
@push('styles')
    <style>
        ...
        .programme-accordion.theme-dg {
            border-left: 6px solid #4fc3f7;
        }

        .programme-accordion.theme-cm {
            border-left: 6px solid #e1306c;
        }

        .programme-accordion.theme-dgcm {
            border-left: 6px solid #f59e0b;
        }

        .programme-accordion.theme-gi {
            border-left: 6px solid #ff9800;
        }

        .programme-accordion.theme-ia {
            border-left: 6px solid #26c6da;
        }

        .programme-accordion.theme-all {
        ...
        .formation-chip.cm {
            background: linear-gradient(135deg, #833AB4 0%, #E1306C 60%, #F56040 100%);
        }
        ...
        @if (!$programmes->isEmpty())
            <div class="tools-card">
                <div class="row g-3 align-items-center">
                    <div class="col-lg-6">
                        <input type="text" id="programmeSearch" class="form-control tools-input"
                            placeholder="Rechercher un programme, une séance, un lieu...">
                    </div>
                    <div class="col-lg-3">
                        <select id="programmeFormationFilter" class="form-select tools-input">
                            ...
                            <option value="Design Graphique & Community Manager">Design Graphique & Community Manager
                            </option>
                    ...
                    <div class="col-lg-3">
                        <select id="programmeMonthFilter" class="form-select tools-input">
                            ...
                            @foreach ($months as $m)
                                <option value="{{ $m }}">{{ \Carbon\Carbon::parse($m)->translatedFormat('F Y') }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                            ...
                            <tbody>
                                @foreach ($programmes as $programme)
                                    @php
                                        ...
                                        $isStudentTargeting =
                                            !empty($programme->student_ids) && is_string($programme->student_ids);
                                        $studentIds = [];
                                    ...
                                    <tr style="border-bottom: 1px solid rgba(51, 65, 85, 0.7);">
                                        <td style="padding: 1rem; vertical-align: middle;">
                                            @if (!empty($monthStart))
                                                <div style="color: #94a3b8;">
                                                    <i
                                                        class="fas fa-calendar me-1"></i>{{ \Carbon\Carbon::parse($monthStart)->translatedFormat('F Y') }}
                                                </div>
                                            @else
                                        ...
                                        <td style="padding: 1rem; vertical-align: middle;">
                                            <div style="display: flex; gap: 0.5rem;">
                                                <button class="btn-toggle btn-sm" type="button" data-bs-toggle="collapse"
                                                    data-bs-target="#prog_table_{{ $programme->id }}" aria-expanded="false"
                                                    style="padding: 0.35rem 0.75rem;">
                                                ...
                                                <form action="{{ route('admin.programmes.destroy', $programme->id) }}"
                                    ...
                                    <tr>
                                        <td colspan="5"
                                            style="padding: 0; border-bottom: 1px solid rgba(51, 65, 85, 0.7);">
                                            <div id="prog_table_{{ $programme->id }}" class="collapse"
                                                style="padding: 1rem; background: rgba(15, 23, 42, 0.3);">
                                                ...
                                                <div style="margin-bottom: 1rem;">
                                                    <div
                                                        style="color: #e2e8f0; font-weight: 600; font-size: 0.9rem; margin-bottom: 0.5rem;">
                                                        <i class="fas fa-bullseye me-2"
                                                            style="color: #4fc3f7;"></i>Détails du ciblage
                                                    </div>
                                                    @if ($isStudentTargeting && !empty($studentIds))
                                                        ...
                                                        @endphp
                                                        <div
                                                            style="padding: 0.5rem; background: rgba(245, 158, 11, 0.1); border-radius: 8px;">
                                                            <div
                                                                style="margin-bottom: 0.5rem; color: #f59e0b; font-weight: 600;">
                                                                <i class="fas fa-user-graduate me-1"></i>Étudiants
                                                                spécifiques ({{ count($studentIds) }})
                                                            </div>
                                                            @foreach ($targetedStudents as $ts)
                                                                <div style="margin-bottom: 0.25rem; color: #94a3b8;">
                                                                    ...
                                                                    @if (!empty($ts->email))
                                                                        <span
                                                                            style="color: #64748b;">({{ $ts->email }})</span>
                                                                    @endif
                                                                </div>
                                                            @endforeach
                                                        </div>
                                                    @else
                                                        <div
                                                            style="padding: 0.5rem; background: rgba(16, 185, 129, 0.1); border-radius: 8px;">
                                                            <div
                                                                style="color: #10b981; font-weight: 600; margin-bottom: 0.25rem;">
                                                                <i class="fas fa-graduation-cap me-1"></i>Formation
                                                                destinataire
                                                            </div>
                                                            <div style="color: #94a3b8; font-size: 1.1rem;">
                                                                <strong
                                                                    style="color: #e2e8f0;">{{ $programme->formation ?? 'Non défini' }}</strong>
                                                            </div>
                                                        </div>
                                                    @endif
                                                </div>
                                                ...
                                                @if (($programme->items ?? collect())->isEmpty())
                                                    <div
                                                        style="padding: 1.5rem 1rem; text-align: center; border: 1px dashed rgba(255,255,255,0.14); border-radius: 12px;">
                                                        <i class="fas fa-inbox"
                                                            style="font-size: 2rem; color: #475569; margin-bottom: 0.5rem;"></i>
                                                        <div style="color: #e2e8f0; font-weight: 600;">Aucune séance</div>
                                                        <div style="color: #94a3b8; font-size: 0.85rem;">Ce programme ne
                                                            contient pas encore de séances.</div>
                                                    </div>
                                                @else
                                                    @php
                                                        $now = now();
                                                                ...
                                                                <tr>
                                                                    <td>
                                                                        ...
                                                                        @if (!empty($it->description))
                                                                            <div class="item-sub">{{ $it->description }}
                                                                            </div>
                                                                        @endif
                                                                    </td>
                                                                    <td>{{ !empty($it->session_date) ? \Carbon\Carbon::parse($it->session_date)->format('d/m/Y') : '' }}
                                                                    </td>
                                                                    <td>{{ !empty($it->session_time) ? \Carbon\Carbon::parse($it->session_time)->format('H:i') : '' }}
                                                                    </td>
                                                                    <td>{{ ($it->type_formation ?? '') === 'presentielle' ? 'Présentielle' : 'En ligne' }}
                                                                    </td>
                                                                    <td>{{ $it->lieu ?? '—' }}</td>
                                                                    <td>
