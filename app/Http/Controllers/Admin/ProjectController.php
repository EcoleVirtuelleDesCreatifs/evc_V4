...
class ProjectController extends Controller
{
    ...
    public function pendingDesignGraphique()
    {
        ...
        $projectsPending = DB::table('projects')
            ...
            ->where(function ($q) {
                $normalizedProgramSql = "REPLACE(REPLACE(REPLACE(REPLACE(LOWER(COALESCE(students.program, '')), ' ', ''), '-', ''), '_', ''), '/', '')";
                $q->whereRaw($normalizedProgramSql . " LIKE ?", ['%design%'])
                    ->whereRaw($normalizedProgramSql . " LIKE ?", ['%graph%'])
                    ->whereRaw($normalizedProgramSql . " NOT LIKE ?", ['%community%']);
            })
    ...
    public function pendingDesignCm()
    {
        ...
        // 3. Récupérer les projets Design en attente / non validés (Design & Community)
        $designProjects = DB::table('design_projects')
            ...
            ->whereNotIn('design_projects.status', ['validated', 'valide', 'termine', 'completed', 'cancelled'])
            ->select(
                'design_projects.id',
                'design_projects.title',
                'design_projects.description',
                DB::raw('NULL as formation'),
        ...
        $allProjects = $tpAssignments->concat($tpLegacy)->concat($designProjects)->concat($cmProjects)->sortByDesc('created_at');
        ...
        $baseQueryDesign = DB::table('design_projects')
            ->leftJoin('users', 'design_projects.user_id', '=', 'users.id')
            ->leftJoin('students', 'users.id', '=', 'students.user_id')
            ->where(function ($q) {
                $q->where('students.program', 'like', '%Design Graphique%')
        ...
        $baseQueryProjects = DB::table('projects')
            ->leftJoin('users', 'projects.user_id', '=', 'users.id')
            ->leftJoin('students', 'users.id', '=', 'students.user_id')
            ->where(function ($q) {
                $q->where('students.program', 'like', '%Design Graphique%')
