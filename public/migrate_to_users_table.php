<?php
/**
 * Migration globale complète vers la table users
 * Unifie toutes les données du profil étudiant dans une seule table cohérente
 */
...
try {
    ...
    echo "🚀 DÉBUT DE LA MIGRATION GLOBALE VERS LA TABLE USERS\n\n";

    // 1. Supprimer l'ancienne table users si elle existe
    echo "1. Suppression de l'ancienne table users...\n";
    $pdo->exec("DROP TABLE IF EXISTS users");
    echo "✅ Ancienne table users supprimée\n\n";

    // 2. Créer la nouvelle table users complète avec tous les champs du profil étudiant
    echo "2. Création de la nouvelle table users complète...\n";
    $createUsersTable = "
    CREATE TABLE users (
    ...
    echo "✅ Nouvelle table users créée avec tous les champs du profil étudiant\n\n";

    // 3. Migrer les données existantes de la table utilisateurs vers users
    echo "3. Migration des données existantes...\n";
    
    // Vérifier si la table utilisateurs existe et contient des données
    $checkUtilisateurs = $pdo->query("SHOW TABLES LIKE 'utilisateurs'");
    if ($checkUtilisateurs->rowCount() > 0) {
        // Récupérer toutes les données de la table utilisateurs
        $stmt = $pdo->query("SELECT * FROM utilisateurs");
        $utilisateurs = $stmt->fetchAll(PDO::FETCH_ASSOC);
        
        if (count($utilisateurs) > 0) {
            echo "   Trouvé " . count($utilisateurs) . " utilisateur(s) à migrer...\n";
            ...
            echo "✅ " . count($utilisateurs) . " utilisateur(s) migré(s) avec succès\n";
        } else {
            echo "   Aucune donnée à migrer dans la table utilisateurs\n";
        }
    } else {
        echo "   Table utilisateurs non trouvée, création de données de test...\n";
    }
    echo "\n";

    // 4. Créer/Recréer les tables associées avec les bonnes relations
    echo "4. Création des tables associées...\n";

    // Table user_activities (activités utilisateur)
    ...
    // Table user_statistics (statistiques utilisateur)
    $pdo->exec("DROP TABLE IF EXISTS user_statistics");
    $createStatsTable = "
    ...
    // Table user_documents (documents CVThèque)
    ...
    // Table user_payments (paiements)
    $pdo->exec("DROP TABLE IF EXISTS user_payments");
    $createPaymentsTable = "
    ...
    // Table sessions (sessions Laravel)
    ...
    // 5. Initialiser les statistiques pour tous les utilisateurs
    echo "5. Initialisation des statistiques utilisateur...\n";
    $pdo->exec("
    ...
    // 6. Créer des données de test si aucun utilisateur n'existe
    $userCount = $pdo->query("SELECT COUNT(*) FROM users")->fetchColumn();
    if ($userCount == 0) {
        echo "6. Création de données de test...\n";
    ...
    // 7. Supprimer l'ancienne table utilisateurs avec gestion des contraintes
    echo "7. Nettoyage des anciennes tables...\n";
    
    // Désactiver temporairement les vérifications de clés étrangères
    $pdo->exec("SET FOREIGN_KEY_CHECKS = 0");
    
    // Supprimer les tables dans l'ordre inverse des dépendances
    $tablesToDrop = ['activites_utilisateur', 'statistiques_utilisateur', 'utilisateurs'];
    foreach ($tablesToDrop as $table) {
        try {
            $pdo->exec("DROP TABLE IF EXISTS {$table}");
            echo "   ✅ Table {$table} supprimée\n";
        } catch (Exception $e) {
            echo "   ⚠️  Table {$table} non trouvée ou déjà supprimée\n";
        }
    }
    
    // Réactiver les vérifications de clés étrangères
    $pdo->exec("SET FOREIGN_KEY_CHECKS = 1");
    echo "✅ Anciennes tables nettoyées\n\n";

    echo "🎉 MIGRATION GLOBALE TERMINÉE AVEC SUCCÈS !\n\n";
